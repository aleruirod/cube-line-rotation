...
void SceneBasic::CreateVBO()
{
    ...
    glBufferData(GL_ARRAY_BUFFER, 108 * sizeof(float) + sizeof(arr), positionData, GL_STATIC_DRAW);
    glBufferSubData(GL_ARRAY_BUFFER, 0, 108 * sizeof(float), positionData);
    glBufferSubData(GL_ARRAY_BUFFER, 108 * sizeof(float), sizeof(arr), arr);

     // bind colorBufferHandle to GL_ARRAY_BUFFER target
    float linecolor[6] = {1.0,1.0,0.0,1.0,1.0,0.0};
    glBindBuffer(GL_ARRAY_BUFFER, colorBufferHandle);
    glBufferData(GL_ARRAY_BUFFER, 108 * sizeof(float) + sizeof(arr), colorData, GL_STATIC_DRAW);
    glBufferSubData(GL_ARRAY_BUFFER, 0, 108 * sizeof(float), colorData);
    glBufferSubData(GL_ARRAY_BUFFER, 108 * sizeof(float), sizeof(arr), linecolor);
...
void SceneBasic::rotateModel(float bX, float bY, float bZ, float dX, float dY, float dZ, float phi) {
    ...
    if (dX != 0.0 || dY != 0.0 || dZ != 0.0) {
        ...
        model = glm::translate(model, vec3(-bX, -bY, -bZ));

        model = glm::rotate(model, phi, vec3(dX, dY, dZ));
